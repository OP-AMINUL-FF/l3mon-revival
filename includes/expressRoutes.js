...
routes.post('/login', async (req, res) => {
    if ('username' in req.body) {
        if ('password' in req.body) {
            ...
            let isMatch = false;
            if (rPassword.length === 32) {
                // Support legacy MD5
                let passwordMD5 = crypto.createHash('md5').update(req.body.password.toString()).digest("hex");
                if (passwordMD5 === rPassword) isMatch = true;
            } else {
                // Use bcrypt
                isMatch = await bcrypt.compare(req.body.password.toString(), rPassword);
            }

            if (req.body.username.toString() === rUsername && isMatch) {
